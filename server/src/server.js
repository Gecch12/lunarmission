...
function serve(req,res){res.setHeader("Cache-Control","no-cache");const u=new URL(req.url||"/",`http://${req.headers.host||"localhost"}`);if(u.pathname==="/health"){res.writeHead(200,{"Content-Type":"application/json"});return res.end(JSON.stringify({status:"ok",game:"Lunar Mission - Artemis II",version:"0.6.1",rooms:rooms.size}))}let rel=decodeURIComponent(u.pathname);if(rel==="/")rel="/index.html";const p=resolve(CLIENT,`.${normalize(rel)}`);if(!p.startsWith(CLIENT)||!existsSync(p)||!statSync(p).isFile()){res.writeHead(404);return res.end("Not found")}res.writeHead(200,{"Content-Type":MIME[extname(p)]||"application/octet-stream"});createReadStream(p).pipe(res)}
...
http.listen(PORT,()=>console.log(`Lunar Mission Artemis II v0.6.1 on ${PORT}`));
